...
object SubscriptionManager {
    ...
    // >>> Hardcoded subscription links (as requested) <<<
    ...
    /** Downloads every subscription URL fresh and returns all parsed configs (deduplicated). */
    suspend fun fetchAll(): List<ProxyConfig> = withContext(Dispatchers.IO) {
        val deferreds = SUBSCRIPTION_URLS.map { url -> async { safeFetch(url) } }
